...
contract SogdiaMallDelivery is ERC165, IERC1155Receiver, ReentrancyGuard {
    ...
    constructor(SogdiaMarketplace marketplace, bytes32 reviewedCatalogHash, uint256[] memory productIds) {
        if (address(marketplace).code.length == 0 || reviewedCatalogHash == bytes32(0)) revert InvalidDelivery();
        market = marketplace; catalogHash = reviewedCatalogHash;
    ...
    function buyAndDeliver(uint256 item, uint256 amount, uint256 expectedTotal, uint64 character) external nonReentrant {
    ...
    function deliverOwned(uint256 item, uint256 amount, uint64 character) external nonReentrant {
